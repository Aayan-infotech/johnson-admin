...
import { Box, Container, IconButton, InputBase, Button, useTheme } from "@mui/material";
...
const OrderManagement = () => {
    ...
    const handleToggleStatus = async (order) => {
        ...
        try {
            await axios.put(`${API_BASE_URL}/order/admin/update-status/${order.id}`, { status: newStatus });
            fetchAllOrders();
        } catch (error) {
    ...
    const handleDelete = async (orderId) => {
        ...
        try {
            ...
            setAllOrdersList((prevList) => prevList.filter((order) => order.id !== orderId));
            setFilteredOrders((prevList) => prevList.filter((order) => order.id !== orderId));
        } catch (error) {
